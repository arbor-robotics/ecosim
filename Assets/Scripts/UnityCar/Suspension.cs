...
[SerializeField] private float vehicleLength = 1.52f;
...
public float[] GetStaticCornerLoad {
	get { return staticCornerLoad; }
}
public float GetRollingRadiusFront {
	get { return rollingRadiusFront; }
}
public float GetRollingRadiusRear {
	get { return rollingRadiusRear; }
}
public float GetTrackFront {
	get { return trackFront; }
}
...
void Start()
{
	...
	// RL RR FL FR
	// wC[0].gameObject.transform.localPosition = new Vector3(trackRear / -2.0f, rollingRadiusRear, -frontBumperRAxleDistance+vehicleLength/2);
	// wC[1].gameObject.transform.localPosition = new Vector3(trackRear / 2.0f, rollingRadiusRear, -frontBumperRAxleDistance+vehicleLength/2);
	// wC[2].gameObject.transform.localPosition = new Vector3(trackFront / -2.0f, rollingRadiusFront, -frontBumperFAxleDistance+vehicleLength/2);
	// wC[3].gameObject.transform.localPosition = new Vector3(trackFront / 2.0f, rollingRadiusFront, -frontBumperFAxleDistance+vehicleLength/2);
